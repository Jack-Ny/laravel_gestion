            ...
            <div class="col-md-12 d-flex align-items-stretch grid-margin">
              <div class="row flex-grow">
                <div class="col-12">
                  <div class="card">
                    <div class="card-body">
                      <h4 class="card-title">Détails <b>{{$data->username}}</b></h4>
                        ...
                        @if(Auth::user()->level == 'admin')
                         <div class="form-group{{ $errors->has('level') ? ' has-error' : '' }}">
                            <label for="level" class="col-md-4 control-label">Catégorie</label>
                            <div class="col-md-6">
                            <select class="form-control" name="level" required="" readonly>
                                <option value="admin" @if($data->level == 'admin') selected @endif>Administrateur</option>
                                <option value="user" @if($data->level == 'user') selected @endif>Abonné</option>
                            </select>
                            </div>
                        </div>
                        @endif
                        <a href="{{route('user.index')}}" class="btn btn-light pull-right">Retour</a>
                    </div>
                  </div>
                </div>
              </div>
